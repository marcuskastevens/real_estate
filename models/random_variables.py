...
class NormalRandomVariable(RandomVariable):
    ...
    def simulate(self, shape: Optional[Tuple[int, int]] = None) -> float:
        ...
        """
        return (
            np.random.normal(loc=self.mu, scale=self.sigma, size=shape)
            if self.sigma
            else self.mu
        )
...
class LogNormalRandomVariable:
    ...
    def simulate(self, shape: Optional[Tuple[int, int]] = None) -> float:
        ...
        return (
            np.exp(
                np.random.normal(
                    loc=self.mu - 0.5 * self.sigma**2, scale=self.sigma, size=shape
                )
            )
            - 1
            if self.sigma
...
class BoundedNormalRandomVariable(RandomVariable):
    ...
    def __init__(
        self, mu: float, sigma: float, lower_bound: float, upper_bound: float
    ) -> None:
    ...
    def simulate(self, shape: Optional[Tuple[int, int]] = None) -> float:
        ...
        if shape:
            ...
            out_of_bounds = (random_variable <= self.lower_bound) | (
                random_variable >= self.upper_bound
            )
            ...
            while np.any(out_of_bounds):
                ...
                random_variable[out_of_bounds] = np.random.normal(
                    loc=self.mu, scale=self.sigma, size=np.sum(out_of_bounds)
                )
...
class UniformRandomVariable(RandomVariable):
    ...
    def simulate(self, shape: Optional[Tuple[int, int]] = None) -> float:
        ...
        return np.random.uniform(
            low=self.lower_bound, high=self.upper_bound, size=shape
        )
...
class IndicatorRandomVariable(RandomVariable):
    ...
    def simulate(self, shape: Optional[Tuple[int, int]] = None) -> float:
        ...
        return np.random.choice(
            a=[0, 1], p=[1 - self.probability, self.probability], size=shape
        )
